...
public class TestSchemaHelper {
  ...
  @Mock private Gossiper mockGossiper;
  ...
  public void before() throws Exception {
    ...
    InetAddressAndPort remoteEndpoint =
        InetAddressAndPort.getByAddress(InetAddress.getByName("127.0.0.2"));
    when(mockGossiper.getLiveMembers())
        .thenReturn(ImmutableSet.of(localEndpoint, remoteEndpoint));

    schemaHelper =
        new SchemaHelper(localEndpoint, mockGossiper, RETRY_INTERVAL_MILLIS);
  }
  ...
  public void testSyncedFromStart() throws UnknownHostException {
    ...
    assertThat(schemaHelper.areSchemasAligned(10 * RETRY_INTERVAL_MILLIS))
        .isTrue();
  }
  ...
  private void givenLocalStateOrder(EndpointState endpointState,
  ...
  private void givenRemoteStateOrder(EndpointState endpointState,
                                     EndpointState... endpointStates)
  ...
  private EndpointState createEndpointStateWithSchema() {
    VersionedValue versionedValue =
        new VersionedValue.VersionedValueFactory(null).schema(
            UUID.randomUUID());

    EndpointState endpointState = mock(EndpointState.class);
    when(endpointState.getApplicationState(eq(ApplicationState.SCHEMA)))
