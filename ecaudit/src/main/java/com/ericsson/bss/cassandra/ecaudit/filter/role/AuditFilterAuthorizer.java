...
public class AuditFilterAuthorizer {
  ...
  static {
    for (String cf :
         Arrays.asList(SystemKeyspace.LOCAL, SystemKeyspace.PEERS_V2, PEERS)) {
      READABLE_SYSTEM_RESOURCES.add(
          DataResource.table(SchemaConstants.SYSTEM_KEYSPACE_NAME, cf));
    }

    ALL_SCHEMA_TABLES.forEach(
        table
        -> READABLE_SYSTEM_RESOURCES.add(
            DataResource.table(SchemaConstants.SCHEMA_KEYSPACE_NAME, table)));
    ALL_VIRTUAL_SCHEMA_TABLES.forEach(
        table
        -> READABLE_SYSTEM_RESOURCES.add(
            DataResource.table(SchemaConstants.VIRTUAL_SCHEMA, table)));
  }

  private IAuthorizer authorizer; // lazy initialization

  public boolean
  isOperationAuthorizedForUser(Permission operation, String user,
                               List<? extends IResource> resourceChain) {
  ...
  isReadingSystemResource(Permission operation,
                          List<? extends IResource> resourceChain) {
  ...
  private synchronized void resolveAuthorizerSync() {
    if (authorizer == null) {
      ...
      if (currentAuthorizer instanceof AuditAuthorizer) {
        // AuditAuthorizer adds the ALTER permission, so we must use the wrapped
        // Authorizer instead
        currentAuthorizer =
            ((AuditAuthorizer)currentAuthorizer).getWrappedAuthorizer();
      }
