 ...
 *
 * The root level "connections" resource represents the collection of all
 * Connection types.
 *
 * "connections" - the root level collection resource
 *
 * Currently only root level resources are represented, but this could be
 * extended to separate between native and JMX connections.
 */
public final class ConnectionResource
    implements IResource, Comparable<ConnectionResource> {
  enum Level { ROOT }

  // permissions which may be granted on the root level resource where EXECUTE
  // represent "connect"
  private static final Set<Permission> ROOT_LEVEL_PERMISSIONS =
      Sets.immutableEnumSet(Permission.AUTHORIZE, Permission.EXECUTE);

  private static final String ROOT_NAME = "connections";
  private static final ConnectionResource ROOT_RESOURCE =
   ...
   */
  public static ConnectionResource root() { return ROOT_RESOURCE; }
  ...
  public static ConnectionResource fromName(String name) {
    ...
    if (!ROOT_NAME.equals(parts[0]) || parts.length > 1) {
      throw new IllegalArgumentException(
          String.format("%s is not a valid connection resource name", name));
    }
  ...
  public String getName() {
    return level == Level.ROOT ? ROOT_NAME
                               : String.format("%s/%s", ROOT_NAME, name);
  }
  ...
  public String toString() {
    return level == Level.ROOT ? "<all connections>"
                               : String.format("<connection %s>", name);
  }
