...
final class AuditAuthKeyspace {
  ...
  private static final String WHITELIST_TABLE_SCHEMA =
      "CREATE TABLE " + WHITELIST_TABLE_NAME_V2 + " ("
      + "role text,"
      + "resource text,"
      + "operations set<text>,"
      + "PRIMARY KEY(role, resource))";
  private static final String WHITELIST_TABLE_DESCRIPTION =
      "audit whitelist assigned to db roles";
  private static final int WHITELIST_TABLE_GC_GRACE_SECONDS =
      (int)TimeUnit.DAYS.toSeconds(90);
  private static final TableMetadata CREATE_ROLE_AUDIT_WHITELISTS =
      CreateTableStatement
          .parse(WHITELIST_TABLE_SCHEMA, SchemaConstants.AUTH_KEYSPACE_NAME)
          .id(TableId.forSystemTable(SchemaConstants.AUTH_KEYSPACE_NAME,
                                     WHITELIST_TABLE_NAME_V2))
  ...
  static KeyspaceMetadata metadata() {
    return KeyspaceMetadata.create(SchemaConstants.AUTH_KEYSPACE_NAME,
                                   ...
                                   Tables.of(CREATE_ROLE_AUDIT_WHITELISTS));
  }
}
