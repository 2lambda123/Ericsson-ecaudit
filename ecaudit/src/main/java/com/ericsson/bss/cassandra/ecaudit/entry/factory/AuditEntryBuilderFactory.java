...
public class AuditEntryBuilderFactory {
  ...
  private static final Logger LOG =
      LoggerFactory.getLogger(AuditEntryBuilderFactory.class);

  // This list of predefined and immutable permission sets are injected to the
  // audit entries created in this factory. They should stay immutable since the
  // {@link AuditEntry} type and its builder doesn't make copies of the sets.
  private static final Set<Permission> ALL_PERMISSIONS = Permission.ALL;
  private static final Set<Permission> USE_PERMISSIONS = ImmutableSet.of(
      Permission.CREATE, Permission.ALTER, Permission.DROP, Permission.SELECT,
      Permission.MODIFY, Permission.AUTHORIZE);
  private static final Set<Permission> SELECT_PERMISSIONS =
      ImmutableSet.of(Permission.SELECT);
  private static final Set<Permission> MODIFY_PERMISSIONS =
      ImmutableSet.of(Permission.MODIFY);
  private static final Set<Permission> CAS_PERMISSIONS =
      ImmutableSet.of(Permission.SELECT, Permission.MODIFY);
  private static final Set<Permission> EXECUTE_PERMISSIONS =
      ImmutableSet.of(Permission.EXECUTE);
  private static final Set<Permission> CREATE_PERMISSIONS =
      ImmutableSet.of(Permission.CREATE);
  private static final Set<Permission> ALTER_PERMISSIONS =
      ImmutableSet.of(Permission.ALTER);
  private static final Set<Permission> DROP_PERMISSIONS =
      ImmutableSet.of(Permission.DROP);
  private static final Set<Permission> DESCRIBE_PERMISSIONS =
      ImmutableSet.of(Permission.DESCRIBE);
  private static final Set<Permission> AUTHORIZE_PERMISSIONS =
      ImmutableSet.of(Permission.AUTHORIZE);

  private final StatementResourceAdapter statementResourceAdapter =
      new StatementResourceAdapter();
  ...
  private Builder createEntryBuilderForUnpreparedStatement(String operation,
                                                           ClientState state) {
    try {
  ...
  /**
   * Delegate builder creation for {@link ParsedStatement}s and {@link
   * CFStatement}s.
   *
  ...
  private Builder createEntryBuilder(Raw parsedStatement) {
    if (parsedStatement instanceof SelectStatement.RawStatement) {
      return createSelectEntryBuilder(
          (SelectStatement.RawStatement)parsedStatement);
    }
    if (parsedStatement instanceof ModificationStatement.Parsed) {
      return createModificationEntryBuilder(
          (ModificationStatement.Parsed)parsedStatement);
    }
    ...
    if (parsedStatement instanceof AuthenticationStatement) {
      return createAuthenticationEntryBuilder(
          (AuthenticationStatement)parsedStatement);
    }
    if (parsedStatement instanceof AuthorizationStatement) {
      return createAuthorizationEntryBuilder(
          (AuthorizationStatement)parsedStatement);
    }
  ...
  public Builder updateBatchEntryBuilder(Builder builder, String operation,
                                         ClientState state) {
  ...
  public Builder updateBatchEntryBuilder(Builder builder,
                                         ModificationStatement statement) {
    return builder
        .permissions(statement.hasConditions() ? CAS_PERMISSIONS
                                               : MODIFY_PERMISSIONS)
        .resource(
            DataResource.table(statement.keyspace(), statement.columnFamily()));
  }
  ...
  private Builder
  createSelectEntryBuilder(SelectStatement.RawStatement statement) {
    return AuditEntry.newBuilder()
        .permissions(SELECT_PERMISSIONS)
  ...
  private Builder
  createModificationEntryBuilder(ModificationStatement statement) {
    return AuditEntry.newBuilder()
        .permissions(statement.hasConditions() ? CAS_PERMISSIONS
                                               : MODIFY_PERMISSIONS)
        .resource(
  ...
  private Builder
  createModificationEntryBuilder(ModificationStatement.Parsed statement) {
    Set<Permission> permissions;
    try {
      boolean hasCondition =
          !((List<?>)FieldUtils.readField(statement, "conditions", true))
               .isEmpty();
  ...
  private Builder createDescribeEntryBuilder(DescribeStatement statement) {
    return AuditEntry.newBuilder()
        ...
        .resource(DataResource.table(statement.getAuditLogContext().keyspace,
                                     statement.getAuditLogContext().scope));
  }
  ...
  private Builder
  createAuthenticationEntryBuilder(AuthenticationStatement statement) {
    ...
    if (statement instanceof RoleManagementStatement) {
      return createRoleManagementEntryBuilder(
          (RoleManagementStatement)statement);
    }
  ...
  private Builder
  createRoleManagementEntryBuilder(RoleManagementStatement statement) {
    return AuditEntry.newBuilder()
  ...
  private Builder
  createAuthorizationEntryBuilder(AuthorizationStatement statement) {
    ...
    if (statement instanceof ListPermissionsStatement) {
      return createListPermissionsEntryBuilder(
          (ListPermissionsStatement)statement);
    }
    if (statement instanceof PermissionsManagementStatement) {
      return createPermissionsManagementEntryBuilder(
  ...
  private Builder
  createListPermissionsEntryBuilder(ListPermissionsStatement statement) {
    return AuditEntry.newBuilder()
        .permissions(AUTHORIZE_PERMISSIONS)
        .resource(statementResourceAdapter.resolveGranteeResource(statement));
  }

  private Builder createPermissionsManagementEntryBuilder(
      PermissionsManagementStatement statement) {
    return AuditEntry.newBuilder()
        .permissions(AUTHORIZE_PERMISSIONS)
  ...
  private Builder createSchemaAlteringEntryBuilder(CQLStatement statement) {
    if (statement instanceof CreateKeyspaceStatement) {
      return createCreateKeyspaceEntryBuilder(
          (CreateKeyspaceStatement)statement);
    }
    ...
    if (statement instanceof CreateFunctionStatement) {
      return createCreateFunctionEntryBuilder(
    ...
    if (statement instanceof CreateAggregateStatement) {
      return createCreateAggregateEntryBuilder(
  ...
  private Builder createSchemaAlteringEntryBuilder(Raw parsedStatement) {
    if (parsedStatement instanceof CreateKeyspaceStatement.Raw) {
      return createCreateKeyspaceEntryBuilder(
          (CreateKeyspaceStatement.Raw)parsedStatement);
    }
    if (parsedStatement instanceof AlterKeyspaceStatement.Raw) {
      return createAlterKeyspaceEntryBuilder(
          (AlterKeyspaceStatement.Raw)parsedStatement);
    }
    if (parsedStatement instanceof DropKeyspaceStatement.Raw) {
      return createDropKeyspaceEntryBuilder(
          (DropKeyspaceStatement.Raw)parsedStatement);
    }

    if (parsedStatement instanceof CreateTableStatement.Raw) {
      return createCreateTableEntryBuilder(
          (CreateTableStatement.Raw)parsedStatement);
    }
    if (parsedStatement instanceof AlterTableStatement.Raw) {
      return createAlterTableEntryBuilder(
          (AlterTableStatement.Raw)parsedStatement);
    }
    if (parsedStatement instanceof DropTableStatement.Raw) {
      return createDropTableEntryBuilder(
          (DropTableStatement.Raw)parsedStatement);
    }

    if (parsedStatement instanceof CreateViewStatement.Raw) {
      return createCreateViewEntryBuilder(
          (CreateViewStatement.Raw)parsedStatement);
    }
    if (parsedStatement instanceof AlterViewStatement.Raw) {
      return createAlterViewEntryBuilder(
          (AlterViewStatement.Raw)parsedStatement);
    }
    ...
    if (parsedStatement instanceof CreateTypeStatement.Raw) {
      return createCreateTypeEntryBuilder(
          (CreateTypeStatement.Raw)parsedStatement);
    }
    if (parsedStatement instanceof AlterTypeStatement.Raw) {
      return createAlterTypeEntryBuilder(
          (AlterTypeStatement.Raw)parsedStatement);
    }
    ...
    if (parsedStatement instanceof CreateFunctionStatement.Raw) {
      return createCreateFunctionEntryBuilder(
          (CreateFunctionStatement.Raw)parsedStatement);
    }
    if (parsedStatement instanceof DropFunctionStatement.Raw) {
      return createDropFunctionEntryBuilder(
          (DropFunctionStatement.Raw)parsedStatement);
    }

    if (parsedStatement instanceof CreateAggregateStatement.Raw) {
      return createCreateAggregateEntryBuilder(
          (CreateAggregateStatement.Raw)parsedStatement);
    }
    if (parsedStatement instanceof DropAggregateStatement.Raw) {
      return createDropAggregateEntryBuilder(
          (DropAggregateStatement.Raw)parsedStatement);
    }

    if (parsedStatement instanceof CreateIndexStatement.Raw) {
      return createCreateIndexEntryBuilder(
          (CreateIndexStatement.Raw)parsedStatement);
    }
    if (parsedStatement instanceof DropIndexStatement.Raw) {
      return createDropIndexEntryBuilder(
          (DropIndexStatement.Raw)parsedStatement);
    }

    if (parsedStatement instanceof CreateTriggerStatement.Raw) {
      return createCreateTriggerEntryBuilder(
          (CreateTriggerStatement.Raw)parsedStatement);
    }
    if (parsedStatement instanceof DropTriggerStatement.Raw) {
      return createDropTriggerEntryBuilder(
          (DropTriggerStatement.Raw)parsedStatement);
    }
  ...
  private Builder
  createCreateKeyspaceEntryBuilder(CreateKeyspaceStatement statement) {
    return AuditEntry.newBuilder()
        .permissions(CREATE_PERMISSIONS)
        .resource(
            DataResource.keyspace(statement.getAuditLogContext().keyspace));
  }

  private Builder
  createCreateKeyspaceEntryBuilder(CreateKeyspaceStatement.Raw statement) {
    return AuditEntry.newBuilder()
        .permissions(CREATE_PERMISSIONS)
  ...
  private Builder
  createAlterKeyspaceEntryBuilder(AlterKeyspaceStatement statement) {
    return AuditEntry.newBuilder()
        .permissions(ALTER_PERMISSIONS)
        .resource(
            DataResource.keyspace(statement.getAuditLogContext().keyspace));
  ...
  private Builder
  createAlterKeyspaceEntryBuilder(AlterKeyspaceStatement.Raw statement) {
    String keyspaceName;
    try {
      keyspaceName =
          (String)FieldUtils.readField(statement, KEYSPACE_NAME, true);
    } catch (IllegalAccessException e) {
      throw new CassandraAuditException("Failed to resolve resource", e);
    }
    return AuditEntry.newBuilder()
  ...
  private Builder
  createDropKeyspaceEntryBuilder(DropKeyspaceStatement statement) {
    return AuditEntry.newBuilder()
        .permissions(DROP_PERMISSIONS)
        .resource(
            DataResource.keyspace(statement.getAuditLogContext().keyspace));
  ...
  private Builder
  createDropKeyspaceEntryBuilder(DropKeyspaceStatement.Raw statement) {
    String keyspaceName;
    try {
      keyspaceName =
          (String)FieldUtils.readField(statement, KEYSPACE_NAME, true);
  ...
  private Builder
  createCreateTableEntryBuilder(CreateTableStatement statement) {
    return AuditEntry.newBuilder()
        .permissions(CREATE_PERMISSIONS)
        .resource(DataResource.table(statement.getAuditLogContext().keyspace,
                                     statement.getAuditLogContext().scope));
  ...
  private Builder
  createCreateTableEntryBuilder(CreateTableStatement.Raw statement) {
    return AuditEntry.newBuilder()
        .permissions(CREATE_PERMISSIONS)
  ...
  private Builder
  createAlterTableEntryBuilder(AlterTableStatement.Raw statement) {
    QualifiedName name;
    try {
  ...
  private Builder
  createDropTableEntryBuilder(DropTableStatement.Raw statement) {
    QualifiedName name;
    try {
      name = (QualifiedName)FieldUtils.readField(statement, NAME, true);
    } catch (IllegalAccessException e) {
  ...
  private Builder
  createCreateViewEntryBuilder(CreateViewStatement.Raw statement) {
    return AuditEntry.newBuilder()
        .permissions(ALTER_PERMISSIONS)
        .resource(statementResourceAdapter.resolveBaseTableResource(statement));
  }
  ...
  private Builder
  createAlterViewEntryBuilder(AlterViewStatement.Raw statement) {
    return AuditEntry.newBuilder()
        .permissions(ALTER_PERMISSIONS)
        .resource(statementResourceAdapter.resolveBaseTableResource(statement));
  }
  ...
  private Builder
  createCreateTypeEntryBuilder(CreateTypeStatement.Raw statement) {
    UTName name;
    try {
  ...
  private Builder
  createAlterTypeEntryBuilder(AlterTypeStatement.Raw statement) {
    UTName name;
    try {
      name = (UTName)FieldUtils.readField(statement, NAME, true);
    } catch (IllegalAccessException e) {
  ...
  private Builder
  createCreateFunctionEntryBuilder(CreateFunctionStatement statement) {
    return AuditEntry.newBuilder()
        .permissions(CREATE_PERMISSIONS)
        .resource(statementResourceAdapter.resolveFunctionKeyspaceResource(
            statement));
  }

  private Builder
  createCreateFunctionEntryBuilder(CreateFunctionStatement.Raw statement) {
    return AuditEntry.newBuilder()
        .permissions(CREATE_PERMISSIONS)
        .resource(statementResourceAdapter.resolveFunctionKeyspaceResource(
            statement));
  ...
  private Builder
  createDropFunctionEntryBuilder(DropFunctionStatement statement) {
    return AuditEntry.newBuilder()
        .permissions(DROP_PERMISSIONS)
  ...
  private Builder
  createDropFunctionEntryBuilder(DropFunctionStatement.Raw statement) {
    return AuditEntry.newBuilder()
        .permissions(DROP_PERMISSIONS)
        .resource(statementResourceAdapter.resolveFunctionResource(statement));
  }
  ...
  private Builder
  createCreateAggregateEntryBuilder(CreateAggregateStatement statement) {
    return AuditEntry.newBuilder()
        .permissions(CREATE_PERMISSIONS)
        .resource(statementResourceAdapter.resolveAggregateKeyspaceResource(
            statement));
  }

  private Builder
  createCreateAggregateEntryBuilder(CreateAggregateStatement.Raw statement) {
    return AuditEntry.newBuilder()
        .permissions(CREATE_PERMISSIONS)
        .resource(statementResourceAdapter.resolveAggregateKeyspaceResource(
            statement));
  ...
  private Builder
  createDropAggregateEntryBuilder(DropAggregateStatement statement) {
    return AuditEntry.newBuilder()
        .permissions(DROP_PERMISSIONS)
  ...
  private Builder
  createDropAggregateEntryBuilder(DropAggregateStatement.Raw statement) {
    return AuditEntry.newBuilder()
        .permissions(DROP_PERMISSIONS)
        .resource(statementResourceAdapter.resolveAggregateResource(statement));
  }
  ...
  private Builder
  createCreateIndexEntryBuilder(CreateIndexStatement statement) {
    return AuditEntry.newBuilder()
        .permissions(ALTER_PERMISSIONS)
        .resource(statementResourceAdapter.resolveBaseTableResource(statement));
  }
  ...
  private Builder
  createCreateIndexEntryBuilder(CreateIndexStatement.Raw statement) {
    return AuditEntry.newBuilder()
        .permissions(ALTER_PERMISSIONS)
        .resource(statementResourceAdapter.resolveBaseTableResource(statement));
  }
  ...
  private Builder
  createDropIndexEntryBuilder(DropIndexStatement.Raw statement) {
    return AuditEntry.newBuilder()
        .permissions(ALTER_PERMISSIONS)
        .resource(statementResourceAdapter.resolveBaseTableResource(statement));
  }
  ...
  private Builder
  createCreateTriggerEntryBuilder(CreateTriggerStatement statement) {
    return AuditEntry.newBuilder()
        .permissions(USE_PERMISSIONS)
        .resource(DataResource.table(statement.getAuditLogContext().keyspace,
                                     statement.getAuditLogContext().scope));
  ...
  private Builder
  createCreateTriggerEntryBuilder(CreateTriggerStatement.Raw statement) {
    QualifiedName name;
    try {
  ...
  private Builder
  createDropTriggerEntryBuilder(DropTriggerStatement statement) {
    return AuditEntry.newBuilder()
        .permissions(USE_PERMISSIONS)
        .resource(DataResource.table(statement.getAuditLogContext().keyspace,
                                     statement.getAuditLogContext().scope));
  ...
  private Builder
  createDropTriggerEntryBuilder(DropTriggerStatement.Raw statement) {
    QualifiedName name;
    try {
      name = (QualifiedName)FieldUtils.readField(statement, TABLE_NAME, true);
    } catch (IllegalAccessException e) {
