...
/**
 * Implements a {@link IRoleManager} that is meant to be paired with {@link
 * AuditPasswordAuthenticator} and
 * {@link AuditAuthorizer}.
 *
 * It provides support for audit white-lists based on role options in Cassandra.
 * This implementation is a decorator which wraps the {@link
 * CassandraRoleManager} for generic role management.
 *
 * An explicit permission check is enforced on ALTER statements. This makes it
 * possible to grant whitelists from one role to another.
 */
public class AuditRoleManager implements IRoleManager {
  private static final Logger LOG =
      LoggerFactory.getLogger(AuditRoleManager.class);
   ...
   * Default constructor.
   *
   * Using this constructor wraps the {@link CassandraRoleManager} for actual
   * role management. But this role manager is paired with the {@link
   * AuditAuthenticator} and it adds support for audit white-list options.
   */
  public AuditRoleManager() {
    this(new CassandraRoleManager(), new AuditWhitelistManager(),
         AuditAdapter.getInstance(), DatabaseDescriptor.getAuthenticator());
  }

  @VisibleForTesting
  AuditRoleManager(IRoleManager wrappedRoleManager,
                   AuditWhitelistManager whitelistManager,
                   AuditAdapter auditAdapter, IAuthenticator authenticator) {
  ...
  public Set<? extends IResource> protectedResources() {
    Set<IResource> combinedSet =
        Sets.newHashSet(wrappedRoleManager.protectedResources());
    combinedSet.add(
