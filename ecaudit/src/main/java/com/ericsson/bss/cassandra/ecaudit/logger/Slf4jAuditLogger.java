...
/**
 * Implements an {@link AuditLogger} that writes {@link AuditEntry} instance
 * into file using {@link Logger}. <br> It is possible to configure a
 * parameterized log message by providing a formatting string {@link
 * Slf4jAuditLoggerConfig#getLogFormat()}.
 */
public class Slf4jAuditLogger implements AuditLogger {
  ...
  public Slf4jAuditLogger(Map<String, String> parameters) {
    this(new Slf4jAuditLoggerConfig(parameters),
  ...
  private LogMessageFormatter<AuditEntry>
  createLogMessageFormatter(Slf4jAuditLoggerConfig auditConfig) {
    try {
  ...
  static Map<String, Function<AuditEntry, Object>>
  getAvailableFieldFunctionMap(Slf4jAuditLoggerConfig auditConfig) {
    return ImmutableMap.<String, Function<AuditEntry, Object>>builder()
        .put("CLIENT_IP",
             entry -> entry.getClientAddress().getAddress().getHostAddress())
        .put("CLIENT_PORT", entry -> getPortOrNull(entry.getClientAddress()))
        .put("COORDINATOR_IP",
             entry -> entry.getCoordinatorAddress().getHostAddress())
        ...
        .put(
            "OPERATION",
            entry
            -> sanitize(entry.getOperation().getOperationString(), auditConfig))
        .put("OPERATION_NAKED",
             entry
             -> sanitize(entry.getOperation().getNakedOperationString(),
                         auditConfig))
        .put("TIMESTAMP", getTimeFunction(auditConfig))
        .put("SUBJECT",
             entry
             -> entry.getSubject()
                    .map(s -> sanitize(s, auditConfig))
                    .orElse(null))
        .build();
  }

  private static String sanitize(String input,
                                 Slf4jAuditLoggerConfig auditConfig) {
    String sanitized = input;
    for (String escapeCharacter : auditConfig.getEscapeCharacters()) {
      sanitized =
          sanitized.replaceAll(escapeCharacter, "\\\\" + escapeCharacter);
    }
    return sanitized;
  }

  private static Integer getPortOrNull(InetSocketAddress address) {
    return address.getPort() == AuditEntry.UNKNOWN_PORT ? null
                                                        : address.getPort();
  }

  static Function<AuditEntry, Object>
  getTimeFunction(Slf4jAuditLoggerConfig auditConfig) {
    return auditConfig.getTimeFormatter()
        .map(Slf4jAuditLogger::getFormattedTimestamp)
        .orElse(AuditEntry::getTimestamp);
  }

  private static Function<AuditEntry, Object>
  getFormattedTimestamp(DateTimeFormatter formatter) {
    return auditEntry
        -> formatter.format(Instant.ofEpochMilli(auditEntry.getTimestamp()));
  }

  @Override
  public void log(AuditEntry logEntry) {
    if (auditLogger.isInfoEnabled()) {
      auditLogger.info(formatter.getLogTemplate(),
                       formatter.getArgumentsForEntry(logEntry));
    }
  }
}
