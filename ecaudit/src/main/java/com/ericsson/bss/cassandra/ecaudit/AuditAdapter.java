...
/**
 * This class will be responsible for populating {@link AuditEntry} instance and
 * passing that to {@link Auditor} instance
 */
public class AuditAdapter {
  ...
  public static AuditAdapter getInstance() { return SingletonHolder.INSTANCE; }

  private static class SingletonHolder {
    private static final AuditAdapter INSTANCE =
        AuditAdapterFactory.createAuditAdapter();
  }

  public void setup() { auditor.setup(); }
  ...
  public void auditRegular(String operation, ClientState state, Status status,
    ...
    if (auditor.shouldLogForStatus(status)) {
      AuditEntry logEntry =
          entryBuilderFactory.createEntryBuilder(operation, state)
   ...
   * @param timestamp the system timestamp for the request
   */
  public void auditPrepare(String operation, ClientState state, Status status,
                           long timestamp) {
    if (auditor.shouldLogForStatus(status) &&
        auditor.shouldLogPrepareStatements()) {
      AuditEntry logEntry =
          entryBuilderFactory.createEntryBuilder(operation, state)
              .client(state.getRemoteAddress())
              .coordinator(FBUtilities.getJustBroadcastAddress())
   ...
   * @param timestamp    the system timestamp for the request
   */
  public void auditPrepared(String rawStatement, CQLStatement statement,
                            ClientState state, QueryOptions options,
                            Status status, long timestamp) {
    if (auditor.shouldLogForStatus(status)) {
      AuditEntry logEntry =
          entryBuilderFactory.createEntryBuilder(statement)
              .client(state.getRemoteAddress())
              .coordinator(FBUtilities.getJustBroadcastAddress())
              .user(state.getUser().getName())
              .operation(new PreparedAuditOperation(rawStatement, options,
                                                    boundValueSuppressor))
              .status(status)
              .timestamp(timestamp)
              .build();
   ...
   * @param rawStatements an ordered list of raw statements associated with the
   ...
   * @param timestamp     the system timestamp for the request
   */
  public void auditBatch(BatchStatement statement, List<String> rawStatements,
                         UUID uuid, ClientState state,
                         BatchQueryOptions options, Status status,
                         long timestamp) {
    if (auditor.shouldLogForStatus(status)) {
      AuditEntry.Builder builder =
          entryBuilderFactory.createBatchEntryBuilder()
              .client(state.getRemoteAddress())
              .coordinator(FBUtilities.getJustBroadcastAddress())
              .user(state.getUser().getName())
              .batch(uuid)
      ...
      if (status == Status.FAILED && auditor.shouldLogFailedBatchSummary()) {
        String failedBatchStatement =
            String.format(BATCH_FAILURE, uuid.toString());
        auditor.audit(
            builder.operation(new SimpleAuditOperation(failedBatchStatement))
                .build());
      } else {
        for (AuditEntry entry : getBatchOperations(
                 builder, statement, rawStatements, state, options)) {
          auditor.audit(entry);
        }
      }
    }
   ...
   * @param status    the status of the operation
   * @param timestamp the system timestamp for the request
   * @throws AuthenticationException if the audit operation could not be
   *     performed
   */
  public void auditAuth(String username, InetAddress clientIp, Status status,
                        long timestamp) throws AuthenticationException {
   ...
   *     performed
   */
  public void auditAuth(String userName, InetAddress clientIp, String subject,
                        Status status, long timestamp) {
    if (auditor.shouldLogForStatus(status)) {
      AuditEntry logEntry =
          entryBuilderFactory.createAuthenticationEntryBuilder()
      ...
      try {
        ...
        throw Exceptions.appendCause(new AuthenticationException(e.toString()),
                                     e);
      }
    }
  }
  ...
  static SimpleAuditOperation statusToAuthenticationOperation(Status status) {
    return new SimpleAuditOperation("Authentication " +
   ...
   */
  private Collection<AuditEntry>
  getBatchOperations(AuditEntry.Builder builder, BatchStatement batchStatement,
                     List<String> rawStatements, ClientState state,
    ...
    for (Object queryOrId : options.getQueryOrIdList()) {
      if (queryOrId instanceof MD5Digest) {
        entryBuilderFactory.updateBatchEntryBuilder(
            builder, batchStatement.getStatements().get(statementIndex));
        builder.operation(new PreparedAuditOperation(
            rawStatements.get(rawStatementIndex++),
            options.forStatement(statementIndex), boundValueSuppressor));
        batchOperations.add(builder.build());
      } else {
        entryBuilderFactory.updateBatchEntryBuilder(
            builder, queryOrId.toString(), state);
