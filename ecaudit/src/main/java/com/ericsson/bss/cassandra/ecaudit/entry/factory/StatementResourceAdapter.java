...
class StatementResourceAdapter // NOPMD
{
  ...
  private static final String FAILED_TO_RESOLVE_BASE_TABLE_OF_FUNCTION =
      "Failed to resolve base table of function";
  private static final String NAME = "name";
  private static final String TABLE_NAME = "tableName";
  private static final String FAILED_TO_RESOLVE_RESOURCE =
   ...
   * <p>
   * The abstract type {@link AuthenticationStatement} itself does not contain
   * any member named "role". But all known implementation does have a member
   * with correct name and type.
   *
  ...
  RoleResource resolveGranteeResource(AuthorizationStatement statement) {
    try {
      RoleResource resource =
          (RoleResource)FieldUtils.readField(statement, "grantee", true);
      if (resource == null) {
  ...
  DataResource resolveKeyspaceResource(UseStatement statement) {
    try {
      String keyspace =
          (String)FieldUtils.readField(statement, "keyspace", true);
  ...
  DataResource resolveBaseTableResource(CreateViewStatement statement) {
    try {
      String baseName =
          (String)FieldUtils.readField(statement, TABLE_NAME, true);
      return DataResource.table(statement.getAuditLogContext().keyspace,
                                baseName);
    } catch (IllegalAccessException e) {
      throw new CassandraAuditException(
          "Failed to resolve base table of view " +
  ...
  DataResource resolveBaseTableResource(CreateViewStatement.Raw statement) {
    try {
      QualifiedName baseName =
          (QualifiedName)FieldUtils.readField(statement, TABLE_NAME, true);
      return DataResource.table(baseName.getKeyspace(), baseName.getName());
    } catch (IllegalAccessException e) {
      throw new CassandraAuditException("Failed to resolve base table of view",
                                        e);
    }
  }
  ...
  DataResource resolveBaseTableResource(AlterViewStatement statement) {
    TableMetadataRef baseTable =
        View.findBaseTable(statement.getAuditLogContext().keyspace,
                           statement.getAuditLogContext().scope);
    if (baseTable == null) {
  ...
  DataResource resolveBaseTableResource(AlterViewStatement.Raw statement) {
    try {
      QualifiedName name =
          (QualifiedName)FieldUtils.readField(statement, NAME, true);
      TableMetadataRef baseTable =
          View.findBaseTable(name.getKeyspace(), name.getName());
      if (baseTable == null) {
  ...
  DataResource resolveBaseTableResource(DropIndexStatement statement) {
    KeyspaceMetadata keyspace = Schema.instance.getKeyspaceMetadata(
        statement.getAuditLogContext().keyspace);

    TableMetadata baseTable =
        null == keyspace
            ? null
            : keyspace.findIndexedTable(statement.getAuditLogContext().scope)
                  .orElse(null);
  ...
  FunctionResource
  resolveFunctionKeyspaceResource(CreateFunctionStatement statement) {
    return FunctionResource.keyspace(statement.getAuditLogContext().keyspace);
  }

  FunctionResource
  resolveFunctionKeyspaceResource(CreateFunctionStatement.Raw statement) {
    try {
      FunctionName name =
          (FunctionName)FieldUtils.readField(statement, NAME, true);
      return FunctionResource.keyspace(name.keyspace);
    } catch (IllegalAccessException e) {
      throw new CassandraAuditException(
          FAILED_TO_RESOLVE_BASE_TABLE_OF_FUNCTION, e);
    }
  }
  ...
  FunctionResource resolveFunctionResource(DropFunctionStatement statement) {
    try {
      ...
      List<CQL3Type.Raw> argRawTypes =
          (List<CQL3Type.Raw>)FieldUtils.readField(statement, ARGUMENTS, true);
      return FunctionResource.functionFromCql(
          statement.getAuditLogContext().keyspace,
          statement.getAuditLogContext().scope, argRawTypes);
    } catch (IllegalAccessException e) {
      throw new CassandraAuditException(
          "Failed to resolve base table of function " +
              statement.getAuditLogContext().keyspace + "." +
  ...
  FunctionResource
  resolveFunctionResource(DropFunctionStatement.Raw statement) {
    try {
      FunctionName name =
          (FunctionName)FieldUtils.readField(statement, NAME, true);
      @SuppressWarnings(UNCHECKED)
      List<CQL3Type.Raw> argRawTypes =
          (List<CQL3Type.Raw>)FieldUtils.readField(statement, ARGUMENTS, true);
      return FunctionResource.functionFromCql(name.keyspace, name.name,
                                              argRawTypes);
    } catch (IllegalAccessException e) {
      throw new CassandraAuditException(
          FAILED_TO_RESOLVE_BASE_TABLE_OF_FUNCTION, e);
  ...
  FunctionResource
  resolveAggregateKeyspaceResource(CreateAggregateStatement statement) {
    return FunctionResource.keyspace(statement.getAuditLogContext().keyspace);
  }

  FunctionResource
  resolveAggregateKeyspaceResource(CreateAggregateStatement.Raw statement) {
    try {
      FunctionName name =
          (FunctionName)FieldUtils.readField(statement, NAME, true);
      return FunctionResource.keyspace(name.keyspace);
  ...
  FunctionResource
  resolveAggregateResource(DropAggregateStatement.Raw statement) {
    try {
      FunctionName name =
          (FunctionName)FieldUtils.readField(statement, NAME, true);
      @SuppressWarnings(UNCHECKED)
