...
public class AuditAuthenticator implements IAuthenticator {
  private static final Logger LOG =
      LoggerFactory.getLogger(AuditAuthenticator.class);
   ...
   *
   * This creates an instance of {@link AuditAuthenticator} with the default
   * {@link AuditAdapter} and gets an {@link IDecoratedAuthenticator} from
   * configuration (or the default one).
   */
  public AuditAuthenticator() {
    this(newWrappedAuthenticator(AuditConfig.getInstance()),
         AuditAdapter.getInstance());
  }

  AuditAuthenticator(IDecoratedAuthenticator wrappedAuthenticator,
  ...
  @VisibleForTesting
  static IDecoratedAuthenticator
  newWrappedAuthenticator(AuditConfig auditConfig) {
  ...
  public SaslNegotiator newSaslNegotiator(InetAddress clientAddress) {
    return new AuditSaslNegotiator(
        clientAddress,
        wrappedAuthenticator.newDecoratedSaslNegotiator(clientAddress));
  }
  ...
  /**
   * Implements a {@link SaslNegotiator} that performs auditing on login
   * attempts.
   */
  private class AuditSaslNegotiator implements SaslNegotiator {
    private final InetAddress clientAddress;
    private final IDecoratedAuthenticator
        .DecoratedSaslNegotiator decoratedSaslNegotiator;

    public AuditSaslNegotiator(InetAddress clientAddress,
                               IDecoratedAuthenticator.DecoratedSaslNegotiator
                                   decoratedSaslNegotiator) {
    ...
    public AuthenticatedUser getAuthenticatedUser()
      ...
      try {
        AuthenticatedUser result =
            decoratedSaslNegotiator.getAuthenticatedUser();
    ...
    private void auditAuth(String userName, InetAddress clientAddress,
                           Status status, long timestamp) {
      Optional<String> subject = decoratedSaslNegotiator.getSubject();
      if (subject.isPresent()) {
        auditAdapter.auditAuth(userName, clientAddress, subject.get(), status,
                               timestamp);
      } else {
