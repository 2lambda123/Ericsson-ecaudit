...
public class AuditRecordReadMarshallable implements ReadMarshallable {
  ...
  public void readMarshallable(WireIn wire) throws IORuntimeException {
    if (auditRecord != null) {
      throw new IORuntimeException(
          "Tried to read from wire with used marshallable");
    }
  ...
  private StoredAuditRecord readV0(WireIn wire) {
    ...
    StoredAuditRecord.Builder builder =
        StoredAuditRecord.builder()
            ...
            .withCoordinatorAddress(
                readInetAddress(wire, WireTags.KEY_COORDINATOR_IP))
  ...
  private StoredAuditRecord readBitmappedRecord(WireIn wire) {
    ...
    fields.ifSelectedRun(Field.TIMESTAMP,
                         ()
                             -> recordBuilder.withTimestamp(
                                 wire.read(WireTags.KEY_TIMESTAMP).int64()));
    fields.ifSelectedRun(Field.CLIENT_IP,
                         ()
                             -> recordBuilder.withClientAddress(readInetAddress(
                                 wire, WireTags.KEY_CLIENT_IP)));
    fields.ifSelectedRun(Field.CLIENT_PORT,
                         ()
                             -> recordBuilder.withClientPort(
                                 wire.read(WireTags.KEY_CLIENT_PORT).int32()));
    fields.ifSelectedRun(
        Field.COORDINATOR_IP,
        ()
            -> recordBuilder.withCoordinatorAddress(
                readInetAddress(wire, WireTags.KEY_COORDINATOR_IP)));
    fields.ifSelectedRun(
        Field.USER,
        () -> recordBuilder.withUser(wire.read(WireTags.KEY_USER).text()));
    fields.ifSelectedRun(Field.BATCH_ID,
                         () -> recordBuilder.withBatchId(readBatchId(wire)));
    fields.ifSelectedRun(Field.STATUS,
                         () -> recordBuilder.withStatus(readStatus(wire)));
    fields.ifSelectedRun(Field.OPERATION,
                         ()
                             -> recordBuilder.withOperation(
                                 wire.read(WireTags.KEY_OPERATION).text()));
    fields.ifSelectedRun(
        Field.OPERATION_NAKED,
        ()
            -> recordBuilder.withNakedOperation(
                wire.read(WireTags.KEY_NAKED_OPERATION).text()));
    fields.ifSelectedRun(Field.SUBJECT,
                         ()
                             -> recordBuilder.withSubject(
                                 wire.read(WireTags.KEY_SUBJECT).text()));
  ...
  private String readV0Type(WireIn wire) throws IORuntimeException {
    ...
    if (!WireTags.VALUE_TYPE_BATCH_ENTRY.equals(type) &&
        !WireTags.VALUE_TYPE_SINGLE_ENTRY.equals(type)) {
  ...
  private InetAddress readInetAddress(WireIn wire, String key)
