...
/**
 * Test reading records stored in the binary log file produced by {@link
 * WriteTestDataUtil} using entry version 2 with the following data: <ul> <li>
 * CLIENT_IP = 0.1.2.3 <li> CLIENT_PORT = 777 <li> COORDINATOR_IP = 4.5.6.7 <li>
 * USER = "bob" <li> BATCH_ID = bd92aeb1-3373-4d6a-b65a-0d60295f66c9 <li> STATUS
 * = SUCCEEDED <li> OPERATION = "SELECT SOMETHING" <li> OPERATION_NAKED =
 * "SELECT SOMETHING NAKED" <li> TIMESTAMP = 1554188832013L <li> SUBJECT =
 * "bob-the-subject
 * </ul>
 * <p>
 * Is written in 4 records with different fields selected:
 * <ul>
 *   <li> record1 - Default fields selected (TIMESTAMP, CLIENT_IP, CLIENT_PORT,
 * COORDINATOR_IP, USER, BATCH_ID, STATUS, OPERATION) <li> record2 - No fields
 * selected <li> record3 - All fields selected (TIMESTAMP, CLIENT_IP,
 * CLIENT_PORT, COORDINATOR_IP, USER, BATCH_ID, STATUS, OPERATION,
 * OPERATION_NAKED, SUBJECT) <li> record4 - Custom fields selected (USER,
 * STATUS, OPERATION_NAKED, SUBJECT)
 * </ul>
 */
public class TestReadVersion2 {
  ...
  public static void beforeClass() {
    ...
    chronicleQueue = SingleChronicleQueueBuilder.single(queueDirVersion1)
                         .blockSize(1024)
                         .readOnly(true)
                         .build();
  ...
  private void readDefault() throws Exception {
    ...
    assertThat(actualAuditRecord.getCoordinatorAddress())
        .contains(InetAddress.getByName("4.5.6.7"));
  ...
  private void readFull() throws Exception {
    ...
    assertThat(actualAuditRecord.getStatus()).contains(Status.SUCCEEDED);
    assertThat(actualAuditRecord.getOperation()).contains("SELECT SOMETHING");
    assertThat(actualAuditRecord.getNakedOperation())
        .contains("SELECT SOMETHING NAKED");
    assertThat(actualAuditRecord.getTimestamp()).contains(1554188832013L);
