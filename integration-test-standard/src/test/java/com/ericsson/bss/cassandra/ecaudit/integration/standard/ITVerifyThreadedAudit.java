...
public class ITVerifyThreadedAudit {
  ...
  public static void beforeClass() throws Exception {
    ...
    try (CqlSession session = cdt.createSession()) {
      ...
      for (int i = 0; i < USER_COUNT; i++) {
        session.execute("CREATE ROLE user" + i +
                        " WITH PASSWORD = 'secret' AND LOGIN = true");
  ...
  public void testValidPreparedStatementsAreLogged() throws Exception {
    ExecutorService executorService =
        Executors.newFixedThreadPool(USER_COUNT * 2);
    List<Future<List<String>>> jobResults = new ArrayList<>();

    for (int i = 0; i < USER_COUNT; i++) {
      jobResults.add(
          executorService.submit(new PreparedStatementClient("user" + i)));
    }

    for (int i = 0; i < USER_COUNT; i++) {
      jobResults.add(
          executorService.submit(new BatchStatementClient("user" + i)));
    }

    for (int i = 0; i < USER_COUNT; i++) {
      jobResults.add(executorService.submit(
          new NonPreparedBatchStatementClient("user" + i)));
    }

    for (int i = 0; i < USER_COUNT; i++) {
      jobResults.add(
          executorService.submit(new PreparedBatchStatementClient("user" + i)));
    }
    ...
    public List<String> call() {
      try (CqlSession privateSession = cdt.createSession(username, "secret")) {
        ...
        PreparedStatement preparedDeleteStatement = privateSession.prepare(
            "DELETE FROM ecks_itvta.ectbl WHERE partk = ?");
  ...
  public class BatchStatementClient implements Callable<List<String>> {
    ...
    public BatchStatementClient(String username) { this.username = username; }
    ...
    public List<String> call() {
      try (CqlSession session = cdt.createSession(username, "secret")) {
        ...
        for (int i = 0; i < 10; i++) {
          ...
          BatchStatement batch =
              BatchStatement.builder(DefaultBatchType.UNLOGGED)
                  .addStatement(
                      preparedInsertStatement1.bind(100 + i, "1", "b1"))
                  .addStatement(
                      preparedInsertStatement1.bind(200 + i, "2", "b2"))
        ...
        return expectedBatchAttemptFragmentsAsUser(expectedStatements,
                                                   username);
      }
    }
  }

  public class NonPreparedBatchStatementClient
      ...
      try (CqlSession session = cdt.createSession(username, "secret")) {
        ...
        for (int i = 0; i < 10; i++) {
          expectedStatements.add(
              batch + String.format("[1234, %d, '1', 'b1', %d, '3', %d, '5']",
                                    100 + i, 200 + i, 300 + i));
          session.execute(preparedBatchStatement.bind(
              1234L, 100 + i, "1", "b1", 200 + i, "3", 300 + i, "5"));
        }
  ...
  private List<String>
  expectedBatchAttemptFragmentsAsUser(List<String> statements, String user) {
