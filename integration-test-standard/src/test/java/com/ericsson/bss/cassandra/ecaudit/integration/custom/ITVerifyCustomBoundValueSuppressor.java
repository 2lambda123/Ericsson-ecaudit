...
/**
 * This test class provides a functional integration test for custom bound value
 * suppressor log format with Cassandra itself. <p> The format configuration is
 * read when the plugin is started and has to be setup before the embedded
 * Cassandra is started. Therefor this test class cannot be run in the same
 * process as the other integration tests (having legacy log formatting). <p>
 * Use simple audit log format - with only OPERATION, to make assertions
 * simpler:
...
public class ITVerifyCustomBoundValueSuppressor {
  ...
  public static void beforeClass() throws Exception {
    ...
    customLogger = new Slf4jAuditLogger(
        Collections.singletonMap("log_format", "operation=${OPERATION}"),
        CUSTOM_LOGGER_NAME);
  ...
  private void executePreparedStatement() {
    ...
    UserDefinedType udt =
        session.getMetadata()
            ...
            .orElseThrow(
                () -> new IllegalArgumentException("Missing UDT definition"));

    PreparedStatement preparedInsert = session.prepare(INSERT);
    session.execute(preparedInsert.bind(
        "PartKey1", 42, "ClusterKey", createBlob(16),
        Arrays.asList(createBlob(4), createBlob(8)),
        Collections.singletonMap(99,
                                 Arrays.asList(createBlob(4), createBlob(4))),
        43, tupleType.newValue("Hello", createBlob(4)),
        udt.newValue()
            .setString("mykey", "Kalle")
            .setByteBuffer("myval", (ByteBuffer)createBlob(16))));
  }
