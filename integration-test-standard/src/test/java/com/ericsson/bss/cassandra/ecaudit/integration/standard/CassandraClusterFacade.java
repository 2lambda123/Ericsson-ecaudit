...
public class CassandraClusterFacade {
  ...
  void setup() {
    ...
    try (CqlSession cassandraSession = cdt.createSession()) {
      cassandraSession.execute(
          "CREATE ROLE " + superName +
          " WITH PASSWORD = 'secret' AND LOGIN = true AND SUPERUSER = true");
      cassandraSession.execute(
          "ALTER ROLE " + superName +
          " WITH OPTIONS = { 'grant_audit_whitelist_for_all' : 'roles'}");
      cassandraSession.execute(
          "ALTER ROLE " + superName +
          " WITH OPTIONS = { 'grant_audit_whitelist_for_all' : 'data'}");
      cassandraSession.execute(
          "ALTER ROLE " + superName +
          " WITH OPTIONS = { 'grant_audit_whitelist_for_all' : 'functions'}");
    }
  ...
  void tearDown() {
    createdUsers.forEach(
        role -> superSession.execute("DROP ROLE IF EXISTS " + role));
    createdKeyspaces.forEach(
        ks -> superSession.execute("DROP KEYSPACE IF EXISTS " + ks));
  ...
  void givenBasicUser(String username) { createUser(username, false); }
  ...
  private void createUser(String username, boolean isSuperuser) {
    superSession.execute(
        ...
        " WITH PASSWORD = 'secret' AND LOGIN = true AND SUPERUSER = " +
        isSuperuser);
  ...
  void givenTable(String table) {
    superSession.execute("CREATE TABLE IF NOT EXISTS " + table +
                         " (key int PRIMARY KEY, value text)");
  }

  void givenTableWithList(String table) {
    superSession.execute("CREATE TABLE IF NOT EXISTS " + table +
                         " (key int PRIMARY KEY, value list<int>)");
  }

  void givenRoleIsWhitelistedForOperationOnResource(String username,
                                                    String operation,
                                                    String resource) {
    superSession.execute("ALTER ROLE " + username +
                         " WITH OPTIONS = {'grant_audit_whitelist_for_" +
                         operation + "' : '" + resource + "'}");
  }
  ...
  void thenAuditLogContainEntryForUser(String auditOperation, String username) {
    ...
    assertThat(loggingEvents.get(0).getFormattedMessage())
        .isEqualTo(expectedAuditEntry(auditOperation, username, "ATTEMPT"));
  }
  ...
  void thenAuditLogContainOnlyAuthenticationAttemptsForUser(String username) {
    ...
    String authenticationAttempt =
        expectedAuditEntry("Authentication attempt", username, "ATTEMPT");
    List<ILoggingEvent> loggingEvents = loggingEventCaptor.getAllValues();
    assertThat(loggingEvents)
        .extracting(ILoggingEvent::getFormattedMessage)
        .allMatch(event -> event.equals(authenticationAttempt));
  }

  void thenAuditLogContainsFailedEntriesForUser(String auditOperation,
                                                String username) {
    ArgumentCaptor<ILoggingEvent> loggingEventCaptor =
        ArgumentCaptor.forClass(ILoggingEvent.class);
    ...
    assertThat(loggingEvents.get(1).getFormattedMessage())
        .isEqualTo(expectedAuditEntry(auditOperation, username, "FAILED"));
  }
  ...
  private String expectedAuditEntry(String auditOperation, String username,
    ...
    return String.format(
        "client:'127.0.0.1'|user:'%s'|status:'%s'|operation:'%s'", username,
        status, obfuscatedOperation);
  }
