...
public class TestLogPrinter {
  ...
  private static final StoredAuditRecord FULL_RECORD =
      mockRecord(123L, "1.2.3.4", 42, "5.6.7.8", "king", Status.ATTEMPT,
  ...
  public void testAuthRecord() {
    ...
    StoredAuditRecord authRecord =
        mockRecord(0L, "1.2.3.4", null, "5.6.7.8", "king", Status.ATTEMPT, null,
    ...
    verify(stream).println(
        eq("0|1.2.3.4|5.6.7.8|king|ATTEMPT|Authentication operation"));
  }
  ...
  public void testAvailableFieldFunctions() {
    ...
    Map<String, Function<StoredAuditRecord, Object>> availableFieldFunctions =
        LogPrinter.getAvailableFieldFunctionMap(configMock);
    StoredAuditRecord emptyRecord = StoredAuditRecord.builder().build();

    assertThat(availableFieldFunctions)
        .containsOnlyKeys("CLIENT_IP", "CLIENT_PORT", "COORDINATOR_IP", "USER",
                          "BATCH_ID", "STATUS", "OPERATION", "OPERATION_NAKED",
                          "TIMESTAMP");

    assertThat(availableFieldFunctions.get("CLIENT_IP").apply(emptyRecord))
        .isNull();
    assertThat(availableFieldFunctions.get("CLIENT_IP").apply(FULL_RECORD))
        .isEqualTo("1.2.3.4");

    assertThat(availableFieldFunctions.get("CLIENT_PORT").apply(emptyRecord))
        .isNull();
    assertThat(availableFieldFunctions.get("CLIENT_PORT").apply(FULL_RECORD))
        .isEqualTo(42);

    assertThat(availableFieldFunctions.get("COORDINATOR_IP").apply(emptyRecord))
        .isNull();
    assertThat(availableFieldFunctions.get("COORDINATOR_IP").apply(FULL_RECORD))
        .isEqualTo("5.6.7.8");

    assertThat(availableFieldFunctions.get("USER").apply(emptyRecord)).isNull();
    assertThat(availableFieldFunctions.get("USER").apply(FULL_RECORD))
        .isEqualTo("king");

    assertThat(availableFieldFunctions.get("BATCH_ID").apply(emptyRecord))
        .isNull();
    assertThat(availableFieldFunctions.get("BATCH_ID").apply(FULL_RECORD))
        .isEqualTo(UUID.fromString("12345678-aaaa-bbbb-cccc-123456789abc"));

    assertThat(availableFieldFunctions.get("STATUS").apply(emptyRecord))
        .isNull();
    assertThat(availableFieldFunctions.get("STATUS").apply(FULL_RECORD))
        .isEqualTo(Status.ATTEMPT);

    assertThat(availableFieldFunctions.get("OPERATION").apply(emptyRecord))
        .isNull();
    assertThat(availableFieldFunctions.get("OPERATION").apply(FULL_RECORD))
        .isEqualTo("select something");

    assertThat(
        availableFieldFunctions.get("OPERATION_NAKED").apply(emptyRecord))
        .isNull();
    assertThat(
        availableFieldFunctions.get("OPERATION_NAKED").apply(FULL_RECORD))
        .isEqualTo("select something - naked");

    assertThat(availableFieldFunctions.get("TIMESTAMP").apply(emptyRecord))
        .isNull();
    assertThat(availableFieldFunctions.get("TIMESTAMP").apply(FULL_RECORD))
        .isEqualTo("123");
  }

  private QueueReader
  givenReaderWithSingleRecord(StoredAuditRecord authRecord) {
  ...
  private LogPrinter givenPrinterWithConfig(EcLogYamlConfig config) {
    return new LogPrinter(ToolOptions.builder().withPath(DEFAULT_PATH).build(),
                          stream, 10, config);
  }

  private QueueReader givenRecords(boolean withBatchId, int count1,
                                   int count2) {
    QueueReader reader = mock(QueueReader.class);
    ...
    for (int i = 0; i < count1 + count2; i++) {
      records.add(mockRecord(
          (long)i, "1.2.3.4", 222, "5.6.7.8", "king", Status.ATTEMPT,
          withBatchId ? UUID.fromString("b23534c7-93af-497f-b00c-1edaaa335caa")
                      : null,
          "SELECT QUERY"));
    }
  ...
  private static StoredAuditRecord mockRecord(Long timestamp, String clientHost,
                                              Integer clientPort,
                                              String coordinatorHost,
                                              String user, Status status,
                                              UUID batchId, String operation) {
    StoredAuditRecord record = mock(StoredAuditRecord.class);
    when(record.getTimestamp()).thenReturn(Optional.ofNullable(timestamp));
    when(record.getClientAddress())
        .thenReturn(Optional.ofNullable(createAddress(clientHost)));
    when(record.getClientPort()).thenReturn(Optional.ofNullable(clientPort));
    when(record.getCoordinatorAddress())
        .thenReturn(Optional.ofNullable(createAddress(coordinatorHost)));
    ...
    when(record.getNakedOperation())
        .thenReturn(Optional.ofNullable(operation).map(op -> op + " - naked"));
    return record;
  }
  ...
  private void verifySingleRecordsFromStartOfSequence(int count) {
    for (int i = 0; i < count; i++) {
      verify(stream).println(
          eq(i + "|1.2.3.4:222|5.6.7.8|king|ATTEMPT|SELECT QUERY"));
    }
  }
